#!/usr/bin/env python3
"""Run one Lepinoc-split fine-tuning case for a given subset size and fold.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one Lepinoc subset fine-tuning case")
    ...
    parser.add_argument(
        ...
        help="Root folder that contains Lepinoc-split.yaml and subsets/",
    )
    parser.add_argument(
        "--project-root",
    ...
    parser.add_argument(
        "--val-common-test",
        action="store_true",
        help="Also validate the trained model on Lepinoc-split test split",
    )
...
def build_paths(args: argparse.Namespace) -> tuple[Path, Path, str]:
    subset_name = f"Lepinoc-split{args.size}-fold{args.fold}"
    dataset_root = Path(args.lepinoc_root)
    dataset_yaml = dataset_root / "subsets" / subset_name / f"{subset_name}.yaml"
...
def main() -> None:
    ...
    if args.val_common_test:
        ...
        model.val(
            data=str(Path(args.lepinoc_root) / "Lepinoc-split.yaml"),
            project=str(project_dir),
