...
def generate_model_comparison_table(csv_path, model_names, metrics, test_datasets=None, output_path=None, caption=None, label=None, float_fmt=2, split_by_dataset=False, combined=False, use_resizebox=False, table_font_size='footnotesize', rotate_headers=False, compact_layout='grouped_columns'):
    """
    ...
    Args:
        ...
        table_font_size (str): Font size command for the table: 'tiny', 'scriptsize', 'footnotesize', 'small', 'normalsize' (default 'footnotesize').
                ...
                compact_layout (str): Layout to use when split_by_dataset=True and combined=True.
                                                            - 'grouped_columns' (default): one dataset group per block of metric columns.
                                                            - 'metric_rows': factorize metrics into rows (Scenario + Metric + dataset columns),
                                                                producing a narrower but taller table.
    ...
    if split_by_dataset and test_dataset_keys is not None and len(test_dataset_keys) > 1:
        if combined:
            ...
            if compact_layout not in {'grouped_columns', 'metric_rows'}:
                raise ValueError("compact_layout must be 'grouped_columns' or 'metric_rows'")
            ...
            if compact_layout == 'grouped_columns':
                ...
                # metric_rows layout: Scenario + Metric + one column per dataset
                # This keeps the table narrow and easier to fit without heavy scaling.
                # Scenario column left, Metric column right, datasets centered.
                # Add a softer light-gray vertical separator between Metric and dataset columns.
                col_spec = 'lr!{\\color{gray!35}\\vrule}' + 'c' * len(test_dataset_keys)
                latex.append(f"\\begin{{tabular}}{{{col_spec}}}")
                latex.append(r"\toprule")

                header_row = [r"\thead{\textbf{Scenario}}", r"\thead{\textbf{Metric}}"]
                for dataset_key in test_dataset_keys:
                    dataset_display = test_dataset_display_names.get(dataset_key, dataset_key)
                    header_row.append(rf"\thead{{\textbf{{{dataset_display}}}}}")
                ...
                for idx, model_key in enumerate(model_keys):
                    ...
                    for metric_idx, m in enumerate(metric_keys):
                        # Zebra stripe by physical row, excluding the first (Scenario) column.
                        # This avoids multirow/rowcolor conflicts and keeps scenario text visible.
                        absolute_row_idx = idx * len(metric_keys) + metric_idx
                        shade_row = (absolute_row_idx % 2 == 0)
                        metric_label = metric_display_names[m]

                        # Vertically center the scenario label across the metric block.
                        scenario_cell = (
                            rf"\multirow{{{len(metric_keys)}}}{{*}}{{{model_display_names[model_key]}}}"
                            if metric_idx == 0 else ''
                        )
                        metric_cell = rf"\cellcolor{{gray!10}} {metric_label}" if shade_row else metric_label
                        parts = [scenario_cell, metric_cell]
                        ...
                        for dataset_key in test_dataset_keys:
                            ...
                            if model_row is not None and not model_row.empty and m in model_row.columns:
                                ...
                                s = ''
                            if shade_row:
                                parts.append(rf"\cellcolor{{gray!10}} {s}")
                            else:
                                parts.append(s)

                        latex.append(' & '.join(parts) + r" \\")
                    # Add a softer separator between scenarios (Metric+datasets columns only).
                    # Keep the stronger custom midrule at the arthro/flatbug boundary.
                    if idx < len(model_keys) - 1:
                        next_is_flatbug_boundary = (
                            separator_index is not None and (idx + 1) == separator_index
                        )
                        if not next_is_flatbug_boundary:
                            last_col = 2 + len(test_dataset_keys)
                            latex.append(rf"\arrayrulecolor{{gray!35}}\cmidrule(lr){{1-{last_col}}}\arrayrulecolor{{black}}")
...
if __name__ == '__main__':
    ...
    csv_path = 'validation/other_datasets_metrics/dataset_perf_comparison.csv'
    ...
    print("\n\n=== Combined table - NARROW (metric rows) ===")
    generate_model_comparison_table(csv_path, scenarios_dict, metrics_dict,
                                  test_datasets=test_datasets_multiple,
                                  split_by_dataset=True, combined=True,
                                  compact_layout='metric_rows',
                                  use_resizebox=False, table_font_size='footnotesize')
