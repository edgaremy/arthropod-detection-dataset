...
plot_multiple_scenarios_bbox_size_metrics <- function(scenario_names, csv_paths, output, metric = 'IoU', test_dataset_name = NULL) {
    # Validate inputs
    ...
    for(i in 1:length(scenario_names)) {
        ...
        if(metric == 'IoU') {
            # Flatten the lists - each bbox gets its own row
            IoUs_with_zeros <- unlist(df$IoUs_with_zeros)
            bbox_relative_sizes <- unlist(df$bbox_sizes)
            metric_values <- IoUs_with_zeros
            bbox_sizes <- bbox_relative_sizes
        } else if(metric == 'F1') {
            # Use average bbox size per image
            bbox_sizes <- sapply(df$bbox_sizes, function(x) ifelse(length(x) > 0, mean(x), 0))
            metric_values <- df$F1
        }
        ...
        scenario_df <- scenario_df[!is.na(scenario_df$bbox_size) & !is.na(scenario_df$metric_value), ]
        
        # Count frequency of each point for this scenario (for point sizing)
        point_counts <- table(paste(round(scenario_df$bbox_size, 3), round(scenario_df$metric_value, 3), sep = "_"))
        scenario_df$count <- as.numeric(point_counts[paste(round(scenario_df$bbox_size, 3), round(scenario_df$metric_value, 3), sep = "_")])
        scenario_df$count[is.na(scenario_df$count)] <- 1
        
        all_plot_data[[i]] <- scenario_df
        
        # Calculate Spearman correlation for this scenario
        valid_idx <- !is.na(scenario_df$metric_value) & !is.na(scenario_df$bbox_size)
        if(sum(valid_idx) > 3) {  # Need at least 4 points for correlation
            X <- scenario_df$bbox_size[valid_idx]
            y <- scenario_df$metric_value[valid_idx]
    ...
    legend_labels <- sapply(sorted_scenarios, function(scenario) {
        ...
        if(length(stats_idx) > 0 && !is.na(correlation_stats[[stats_idx]]$rho)) {
            sprintf("%s (ρ=%.2f, p=%.3f)", 
                    scenario, 
                    correlation_stats[[stats_idx]]$rho, 
                    correlation_stats[[stats_idx]]$p_value)
        } else {
            sprintf("%s (ρ=NA, p=NA)", scenario)
        }
    })
    ...
    if(metric == 'IoU') {
        xlabel <- "Arthropod relative size"
    } else {
        xlabel <- "Arthropod relative size (averaged per image)"
    }
    ...
    # Create the plot
    p <- ggplot(plot_df, aes(x = bbox_size, y = metric_value, color = scenario, fill = scenario)) +
...
plot_bbox_size_metrics <- function(csv_path, output, metric = 'IoU') {
    # Read the CSV file
    ...
    if (metric == "IoU") {
        ...
        # Use average bbox size per image
        bbox_relative_sizes <- sapply(df$bbox_sizes, function(x) ifelse(length(x) > 0, mean(x), 0))
        metric_values <- df$F1
    ...
    # Remove rows with invalid data
    plot_df <- plot_df[!is.na(plot_df$bbox_size) & !is.na(plot_df$metric_value), ]
    
    # Count frequency of each point
    point_counts <- table(paste(round(plot_df$bbox_size, 3), round(plot_df$metric_value, 3), sep = "_"))
    plot_df$count <- as.numeric(point_counts[paste(round(plot_df$bbox_size, 3), round(plot_df$metric_value, 3), sep = "_")])
    plot_df$count[is.na(plot_df$count)] <- 1
    
    # Calculate Spearman correlation
    valid_idx <- !is.na(plot_df$metric_value) & !is.na(plot_df$bbox_size)
    X <- plot_df$bbox_size[valid_idx]
    y <- plot_df$metric_value[valid_idx]
    cor_test <- cor.test(X, y, method = "spearman")
    rho <- cor_test$estimate
    p_value <- cor_test$p.value
    
    # Use paletteer for consistent color scheme
    if (!requireNamespace("paletteer", quietly = TRUE)) {
        install.packages("paletteer")
    }
    library(paletteer)
    jewel_colors <- paletteer_d("ggthemes::Jewel_Bright", 2)
    ...
    # Create the plot
    p <- ggplot(plot_df, aes(x = bbox_size, y = metric_value)) +
        geom_point(aes(size = 20 * log2(count + 1), color = "Data points"), alpha = 0.3) +
        geom_smooth(aes(color = fit_name), 
                   method = smooth_method, formula = smooth_formula, 
                   linewidth = 1.2, se = TRUE, fill = jewel_colors[2], alpha = 0.2) +
        ...
        scale_color_manual(
            name = "", 
            values = setNames(c(jewel_colors[1], jewel_colors[2], "transparent", jewel_colors[2]), 
        ...
        guides(
            color = guide_legend(
                override.aes = list(
                    ...
                    fill = c(NA, NA, NA, jewel_colors[2])          # No fill, no fill, no fill, jewel fill
                )
            )
        ) +
        theme_minimal() +
        theme(
            ...
            legend.margin = margin(4, 8, 8, 8),   # Reduced top margin from 8 to 4
            legend.text = element_text(size = 15),  # Bigger font: increased from 13 to 15
            legend.key = element_rect(fill = "#f0f0f0", color = NA),  # Grey background for keys
            legend.key.size = unit(1.4, "cm"),     # Bigger icons: increased from 1.2cm to 1.4cm
            legend.spacing.y = unit(0.2, "cm"),    # Slightly reduced spacing to compensate for bigger elements
...
result <- plot_multiple_scenarios_bbox_size_metrics(
    scenario_names = scenario_names,
    csv_paths = csv_paths,
    output = 'validation/flatbug/plots/compare_bbox_size_F1.png',
    metric = 'F1',
...
result <- plot_multiple_scenarios_bbox_size_metrics(
    scenario_names = scenario_names,
    csv_paths = csv_paths,
    output = 'validation/flatbug/plots/compare_bbox_size_IoU.png',
    metric = 'IoU',
    test_dataset_name = "ArthroNat"
)
...
result <- plot_multiple_scenarios_bbox_size_metrics(
    scenario_names = scenario_names,
    csv_paths = csv_paths,
    output = 'validation/flatbug/plots/compare_bbox_size_F1(fb).png',
    metric = 'F1',
...
result <- plot_multiple_scenarios_bbox_size_metrics(
    scenario_names = scenario_names,
    csv_paths = csv_paths,
    output = 'validation/flatbug/plots/compare_bbox_size_IoU(fb).png',
    metric = 'IoU',
    test_dataset_name = "flatbug"
)